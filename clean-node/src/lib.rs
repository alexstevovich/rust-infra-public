//! Filesystem cleanup for Node projects identified by `package.json`.
...
/// Removes `node_modules` from the root Node project, or all discovered projects when recursive.
    ...
    for project in roots {
        ...
        match fs::metadata(&marker) {
            Ok(metadata) if metadata.is_file() => {
                remove_directory(project.join("node_modules"), dry_run, &mut report)
            }
            Ok(_) => {}
