...
fn main() -> ExitCode {
    ...
    let (Some(source), Some(target), None) = (args.next(), args.next(), args.next()) else {
        eprintln!("Usage: comfyui-meta-copy <source> <target>");
    ...
    match copy(Path::new(&source), Path::new(&target)) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("comfyui-meta-copy: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn copy(source: &Path, target: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let Some(metadata) = comfyui_meta::extract(&source)? else {
        return Ok(());
    };
    let target_bytes = fs::read(target)?;
    let updated = comfyui_meta::embed(&target_bytes, &metadata)?;
